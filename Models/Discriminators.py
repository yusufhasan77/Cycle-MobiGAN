...
### Define the critic
# Define the PatchGAN discriminator
class CycleWGAN_GP_Critic(nn.Module):
    '''
    Similar to the discriminator as in CycleGAN paper.
    Instance Norm layers have been removed and returns a vector instead of a matrix as in PatchGAN.
    Critic Class. Takes an image as input and outputs a vector of values indicating how real/fake the image is.
    Inputs: input_channels -> The number of channels in the input (e.g 3 for RBG image)
            initial_channels -> The number of channels for the first layer of the discriminator
    '''
    ...
    def __init__(self, input_channels, initial_channels=64):
        ...
        self.downsample = nn.Sequential(
            # Define the first downsampling layer (output n*64*128*128)
            CriticBlock(initial_channels, initial_channels,kernel_size = 4, stride = 2,activation="leakyrelu"),
            # Define the second downsampling layer (output n*128*64*64)
            CriticBlock(initial_channels, initial_channels * 2, kernel_size = 4, stride = 2,activation="leakyrelu"),
            # Define the third downsampling  layer (output n*256*32*32)
            CriticBlock(initial_channels * 2, initial_channels * 4, kernel_size = 4, stride = 2,activation="leakyrelu"),
            # Define the fourth downsampling  layer (output n*512*31*31)
            CriticBlock(initial_channels * 4, initial_channels * 8, kernel_size = 4, stride = 1,activation="leakyrelu"),
        )
        # Define the final downsampling  layer
        self.final = nn.Conv2d(initial_channels * 8, 1, kernel_size=4, stride = 1, padding = 1)
    ...
    def forward(self, x):
        ...
        # Apply the first convolution to increase channels (e.g output n*64*256*256)
        x = self.initial(x)
        # Apply the downsampling layers
        x = self.downsample(x)
        # Apply the final convolution and  (e.g output n*1*30*30)
        x = self.final(x)
        # Flatten the output into one long tensor and return it
        return x.view(len(x), -1)
###

### Define the critic with depthwise convolution layers
# Define the PatchGAN discriminator
class CycleWGAN_GP_Critic_DWS(nn.Module):
    '''
    Similar to the discriminator as in CycleGAN paper,
    all convolution layers have been replaced with depthwise seperable convoltions,
    returns a vector instead of a matrix produced by PatchGAN.
    Critic Class with DW seperable convolution layers. Takes an image as input and outputs a vector of values indicating how real/fake the image is.
    Inputs: input_channels -> The number of channels in the input (e.g 3 for RBG image)
            initial_channels -> The number of channels for the first layer of the discriminator
    '''
